...
class ConvNextClassifier(pl.LightningModule):
    ...
    def __init__(
        ...
        self.convnext_model = models.convnext_tiny(pretrained=use_transfer)

        # Replace old FC layer with Identity so we can train our own
        linear_size = list(self.convnext_model.children())[-1].in_features
        # replace final layer for fine tuning
        self.convnext_model.fc = nn.Linear(linear_size, num_classes)



        if tune_fc_only:  # option to only tune the fully-connected layers
            for child in list(self.convnext_model.children())[:-1]:
